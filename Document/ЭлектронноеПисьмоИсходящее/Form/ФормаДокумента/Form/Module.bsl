...
Процедура УправлениеДоступностью()
	...
	Если Объект.СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Исходящее") И 
		(НЕ ИнформационнаяБазаФайловая
...
Процедура ЗаполнитьВложения(Параметры = Неопределено)
	...
	Если Объект.Ссылка.Пустая() И Параметры <> Неопределено Тогда
		Если Параметры.Свойство("Основание") И 
			...
			Параметры.Основание.Свойство("Команда") И 
