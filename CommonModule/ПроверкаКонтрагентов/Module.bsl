...
Процедура ПроверитьКонтрагентаФоновоеЗадание(Параметры) Экспорт 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Проверка контрагента из карточки контрагента'", КодОсновногоЯзыка),
...
Процедура ПроверитьКонтрагентовПослеВключенияПроверкиФоновоеЗадание() Экспорт
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Проверка контрагентов в фоновом задании'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПроверитьКонтрагентовВДокументеФоновоеЗадание(Форма, ПараметрыФоновогоЗадания) Экспорт
	...
	Если ДанныеКонтрагентов.Количество() > 0 Тогда
		...
		Исключение
			...
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Проверка контрагентов фоновым заданием в документе'", КодОсновногоЯзыка),
				УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ОтобразитьРезультатПроверкиКонтрагентаВДокументе(Форма) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется Тогда
		...
		ПроверкаКонтрагентовПереопределяемый.ОтобразитьРезультатПроверкиКонтрагентаВДокументе(Форма, СтандартнаяОбработка);
		...
		Если ЗначениеЗаполнено(Форма.РеквизитыПроверкиКонтрагентов.АдресХранилища) И 
			ЭтоАдресВременногоХранилища(Форма.РеквизитыПроверкиКонтрагентов.АдресХранилища) Тогда
...
Функция ВыводитьСтрокуОтчета(СтруктураПараметров, Контрагент, ИНН, КПП, Дата) Экспорт
	...
	СтандартнаяОбработка 	= Истина;
	ПроверкаКонтрагентовПереопределяемый.ОпределитьНужноЛиВыводитьСтрокуОтчета(
	...
	ВыводитьСтроку = НедействующиеКонтрагенты.НайтиСтроки(
		Новый Структура("Контрагент, ИНН, КПП, Дата", Контрагент, ИНН, КПП, НачалоДня(Дата))).Количество() > 0;
	...
	Если ЕстьДоступКВебСервисуФНС() Тогда
		...
		Если НедействующиеКонтрагентыОпределены(ПроверяемыеКонтрагенты) Тогда
			...
			Если ПроверяемыеКонтрагенты.Количество() > 0 Тогда
				...
				ПроверкаКонтрагентовПереопределяемый.СформироватьОтчет(СтруктураПараметров, ПроверкаКонтрагентовОтчетПоНекорректнымКонтрагентам);
...
Процедура ОтобразитьРезультатПроверкиКонтрагентаВОтчете(Форма, ПолеТабличногоДокумента, РезультатФормированияОтчета) Экспорт
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.ОтобразитьРезультатПроверкиКонтрагентаВОтчете(Форма, СтандартнаяОбработка);
...
Процедура ВывестиОтчет(Форма, ПолеТабличногоДокумента, РезультатФормированияОтчета) Экспорт
	...
	ПроверкаКонтрагентовПереопределяемый.ВывестиОтчет(Форма, ПолеТабличногоДокумента, РезультатФормированияОтчета, СтандартнаяОбработка);
...
Процедура ПриСозданииНаСервереКонтрагент(Форма, Параметры) Экспорт
	...
	Если ПроверкаКонтрагентовИспользуется Тогда
		...
		Иначе
			...
			Если (ЗначениеЗаполнено(КонтрагентСсылка) ИЛИ ЗначениеЗаполнено(Параметры.ЗначениеКопирования))
...
Процедура ПриСозданииНаСервереСписокКонтрагентов(Список) Экспорт
	...
	Состояния.Добавить(Перечисления.СостоянияСуществованияКонтрагента.КППНеСоответствуетИНН);
...
Процедура УдалитьРезультатПроверкиКонтрагентов(Контрагенты = Неопределено) Экспорт 
	...
	Исключение
		...
		НСтр("ru = 'Проверка контрагентов.Очистка предыдущих результатов проверок'", КодОсновногоЯзыка), 
...
Процедура УдалитьРезультатПроверкиКонтрагентовПоДокументам(Документы = Неопределено) Экспорт 
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Проверка контрагентов.Очистка предыдущих результатов проверок контрагентов в документах'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПроксиСервиса()
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Проверка контрагентов.Ошибка доступа к веб-сервису проверки контрагентов'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Проверка контрагентов.Запись результатов проверки контрагентов в документе в регистр'", КодОсновногоЯзыка), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПодготовитьДанныеКПроверке(ДанныеКонтрагентовДляПроверки)
	...
	Для каждого ДанныеКонтрагентаДляПроверки Из ДанныеКонтрагентовДляПроверки Цикл
		...
		Если ДанныеКонтрагентовДляПроверки.Колонки.Найти("ОбластьДанныхВспомогательныеДанные") = Неопределено 
			ИЛИ НЕ ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
...
Процедура ПодготовитьДанныеКПроверкеПоКаждомуКонтрагенту(ДанныеКонтрагента)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если ЭтоИностранныйКонтрагент(ДанныеКонтрагента.Контрагент)Тогда
...
Функция ЭтоИностранныйКонтрагент(КонтрагентСсылка)
	
	ЯвляетсяИностранным = Ложь;
	ПроверкаКонтрагентовПереопределяемый.ОпределитьЯвляетсяЛиКонтрагентИностранным(КонтрагентСсылка, ЯвляетсяИностранным);
...
Функция СостояниеНаОсновеОтветаСервиса(ДанныеКонтрагента, Ответ)
	...
	Иначе
		Состояние = Перечисления.СостоянияСуществованияКонтрагента.СодержитОшибкиВДанных;
	КонецЕсли;
...
Процедура ПроверитьНепроверенныхКонтрагентов(ЭтоОбновлениеИБВМоделиСервиса, Параметры = Неопределено)
	...
	Текст = 
		...
		|	&Дата КАК Дата
		|ИЗ
		|	Справочник.%Имя КАК Контрагенты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияКонтрагентов КАК СостоянияКонтрагентов
	...
	Текст = СтрЗаменить(Текст, "%ИНН", СвойстваСправочника.ИНН);
	Текст = СтрЗаменить(Текст, "%КПП", СвойстваСправочника.КПП);
	Текст = СтрЗаменить(Текст, "%Имя", СвойстваСправочника.Имя);
	
	Если СправочникКонтрагентовИерархический() Тогда
		Текст = СтрЗаменить(Текст, "%Условие", "И Контрагенты.ЭтоГруппа = ЛОЖЬ");
	Иначе
...
Процедура ПоместитьТаблицуЗначенийВоВременнуюТаблицу(Таблица, Запрос, ИмяВременнойТаблицы, ПостфиксСинонима = "")
...
Функция ПредставлениеКолонок(Таблица, СинонимТаблицы = "", ПостфиксСинонима = "")
...
Процедура ДополнитьДанныеКонтрагентовВДокументе(
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		...
		СвойстваСправочника = ПроверкаКонтрагентовКлиентСервер.СвойстваСправочникаКонтрагенты();
		РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, СвойстваСправочника.ИНН + "," + СвойстваСправочника.КПП);
		ЗаполнитьЗначенияСвойств(СтрокаОписания, РеквизитыКонтрагента);
...
Функция ОписаниеДокументов(Форма)
	...
	Если ПроверкаКонтрагентовКлиентСервер.ЭтоДокументСКонтрагентомВШапке(Форма) Тогда
		...
		ПроверкаКонтрагентовПереопределяемый.СформироватьОписаниеДокументовСКонтрагентомВШапке(Форма, Описание);
	...
	Если ПроверкаКонтрагентовКлиентСервер.ЭтоДокументСоСчетомФактуройВПодвале(Форма) Тогда
		...
		Описание = ШаблонТаблицыОписания();
		ПроверкаКонтрагентовПереопределяемый.СформироватьОписаниеДокументовСоСчетомФактуройВПодвале(Форма, Описание);
	...
	Если ПроверкаКонтрагентовКлиентСервер.ЭтоСчетФактура(Форма) Тогда
		...
		Описание = ШаблонТаблицыОписания();
		ПроверкаКонтрагентовПереопределяемый.СформироватьОписаниеСчетовФактур(Форма, Описание);
...
Функция ОписаниеКонтрагентовВСчетеФактуре(Тип)
	...
	ПроверкаКонтрагентовПереопределяемый.ПолучитьИменаРеквизитовКонтрагентовВСчетеФактуре(Тип, ИменаРеквизитовКонтрагентов);
...
Функция ОписаниеТабличныхЧастей(Форма) Экспорт
	...
	ПроверкаКонтрагентовПереопределяемый.СформироватьОписаниеДокументовСТабличнымиЧастями(Форма, ОписаниеТабличныхЧастей);
...
Процедура УстановитьУсловноеОформлениеСпискаДокументов(Список)
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.УстановитьУсловноеОформлениеСпискаДокументов(Список, СтандартнаяОбработка);
		...
		Для каждого ОписаниеТабличнойЧасти Из ОписаниеТабличныхЧастей Цикл
			...
			Для каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
				...
				СтрокиДанныхКонтрагентов = ДанныеКонтрагентов.НайтиСтроки(
					Новый Структура("Ссылка, Контрагент, Дата", ДокументСсылка, Контрагент, Дата));
...
Процедура ПрорисоватьСостоянияКонтрагентовВДокументе(Форма, СостояниеПроверки, ДополнительныеПараметры = Неопределено)
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.ПрорисоватьСостоянияКонтрагентовВДокументе(Форма, СостояниеПроверки, 
...
Функция ДанныеКонтрагентовРазличающиесяМеждуКэшемИСервисомФНС(ДанныеКонтрагентов, ДанныеКонтрагентовИзКэша)
	...
	ПоместитьТаблицуЗначенийВоВременнуюТаблицу(ДанныеКонтрагентовИзКэша, Запрос, "ДанныеКонтрагентовИзКэша", "ИзКэша");
	ПоместитьТаблицуЗначенийВоВременнуюТаблицу(ДанныеКонтрагентов,       Запрос, "ДанныеКонтрагентов");
	
	// В таблице оставляем строки с различающимся состоянием
	Запрос.Текст = Запрос.Текст + "
	                              |ВЫБРАТЬ
	                              |	" + ПредставлениеКолонок(ДанныеКонтрагентовИзКэша, "ДанныеКонтрагентовИзКэша.", "ИзКэша") + ",
	                              |	" + ПредставлениеКолонок(ДанныеКонтрагентов, "ДанныеКонтрагентов.") + "
	                              |ИЗ
...
Процедура УстановитьУсловноеОформлениеТабличнойЧастиДокумента(Форма)
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.УстановитьУсловноеОформлениеТабличнойЧастиДокумента(Форма, СтандартнаяОбработка);
...
Процедура ДобавитьКолонкиТабличнойЧастиСРезультатамиПроверкиВТабличныеЧасти(Форма)
	...
	Для каждого ОписаниеТабличнойЧасти Из ОписаниеТабличныхЧастей Цикл
		...
		ЕстьЭтоНекорректныйКонтрагент 	= ЕстьРеквизитФормы(ТабличнаяЧасть, "ЭтоНекорректныйКонтрагент");
...
Процедура ДобавитьКолонкиТаблицыФормыСРезультатамиПроверкиВТабличныеЧасти(Форма)
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.ДобавитьКолонкиТаблицыФормыСРезультатамиПроверкиВТабличныеЧасти(Форма, СтандартнаяОбработка);
	...
	Для каждого ОписаниеТабличнойЧасти Из ОписаниеТабличныхЧастей Цикл
		...
		ПолеСКонтрагентом  	= Форма.Элементы[ОписаниеТабличнойЧасти.ИмяПоляСКонтрагентом];
		НовоеПолеКартинки 	= Форма.Элементы.Найти(ПроверкаКонтрагентовКлиентСервер.ИмяПоляКартинки(ТаблицаФормы));
		НоваяГруппаКолонок 	= Форма.Элементы.Найти(ПолеСКонтрагентом.Имя + "Группа");
...
Функция ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеКонтрагентов, Отбор = Неопределено)
...
//
// Параметры:
//  СчетФактураОбъект	 - ДанныеФормыСтруктура - счет-фактура, для которой необходимо определить заполненность
//                                              контрагентов.
// Возвращаемое значение:
// Булево - Истина, если заполнен хотя бы один контрагент.
Функция ВСчетеФактуреЗаполненХотяБыОдинКонтрагент(СчетФактураОбъект)
...
Процедура УстановитьУсловноеОформлениеСпискаКонтрагентов(Список)
	...
	СтандартнаяОбработка = Истина;
	ПроверкаКонтрагентовПереопределяемый.УстановитьУсловноеОформлениеСпискаКонтрагентов(Список, СтандартнаяОбработка);
	...
	// ПроверкаКонтрагентовСостояние Не равно "Перечисление.СостоянияСуществованияКонтрагента.ПрекратилДеятельность".
	Отбор = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	...
	Отбор.ПравоеЗначение 	= Перечисления.СостоянияСуществованияКонтрагента.ПрекратилДеятельность;
	...
	// ПроверкаКонтрагентовСостояние Равно "Перечисление.СостоянияСуществованияКонтрагента.ПрекратилДеятельность"
	Отбор = НовоеУсловноеОформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения 		= ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("ПроверкаКонтрагентовСостояние");
...
Процедура ВыделитьКонтрагентовСОшибкамиВОтчете(НедействующиеКонтрагенты, ТабличныйДокумент) Экспорт
	...
	Для каждого СтрокаСНедействующимКонтрагентом Из НедействующиеКонтрагенты Цикл
		...
		ПроверкаКонтрагентовПереопределяемый.ВыделитьКонтрагентаСОшибкойВСтрокеОтчета(ТабличныйДокумент, 
