...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВебКлиент Тогда
		...
		Элементы.ПояснениеДляВыбораСправочникаДляЗагрузки.Заголовок = НСтр("ru = 'Выбор справочника для загрузки данных из электронных таблиц, расположенных во внешних файлах (например: Microsoft Office Excel, OpenOffice Calc и др.).'");
		ВидЗагрузкиДанных = 1;
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Обработки.ЗагрузкаДанныхИзФайла.ИнициализироватьЗагрузкуВТабличнуюЧасть(ИмяОбъектаСопоставления, Параметры.ИмяМакетаСШаблоном, ТаблицаИнформацияПоКолонкам, ШаблонСДанными, ДополнительныеПараметры, Отказ);
			Если Отказ Тогда
				Возврат;
			КонецЕсли;
...
Процедура ТаблицаСопоставленияДанныхПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Иначе
		...
		Для каждого КолонкаТаблицы Из КолонкиОбязательнаДляЗаполнения Цикл
			Если НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные["ТЧ_" + КолонкаТаблицы.Ассоциация]) Тогда
				РезультатСопоставленияСтроки = "НеСопоставлен";
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.Заголовок = НСтр("ru = 'Загрузка данных в справочник ""'") + ПредставлениеСправочника(ИмяОбъектаСопоставления)+"""";
		ЭтотОбъект.АвтоЗаголовок = Ложь;
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхНаСервере(ФоновоеЗадание = Ложь)
	...
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		Исключение
			// Используется старый вариант метода СопоставитьЗагружаемыеДанные без параметра ДополнительныеПараметры.
...
Процедура ПоместитьДанныеВТаблицуСопоставления(АдресЗагруженныхДанных, АдресКопииТабличнойЧасти, СписокНеоднозначностей)
	...
	Для каждого КолонкаТаблицы Из ОтобранныеКолонкиОбязательнаДляЗаполненияТаблицы  Цикл
		КолонкиОбязательныеДляЗаполнения.Вставить(КолонкаТаблицы.Ассоциация, Истина);
	КонецЦикла;
		...
		Если Строки.Количество() > 0 Тогда 
			...
			Для каждого Колонка Из ЗагружаемыеДанные.Колонки Цикл
				Если Колонка.Имя <> "Идентификатор" И Колонка.Имя <> "РезультатСопоставленияСтроки" И Колонка.Имя <> "ОписаниеОшибки" 
					...
					Если КолонкиТабличнойЧасти.Получить(Колонка.Имя) <> Неопределено И ЗначениеЗаполнено(Строка[Колонка.Имя]) 
							И НЕ ЗначениеЗаполнено(НоваяСтрока["ТЧ_" + Колонка.Имя]) Тогда
						НоваяСтрока["РезультатСопоставленияСтроки"] = "НеСопоставлен";
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		...
		ТекстПодсказки = ТекстПодсказки  + НСтр("ru = 'Затем загрузить заполненную таблицу в одном из форматов: Книга Microsoft Excel (.xlsx), текст с разделителями (.csv) или табличный документ (.mxl).'") + Символы.ПС;
	Иначе
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахДляТЧ() 
	...
	Для Каждого Колонка Из ВременнаяТЗ.Колонки Цикл
		...
		НовыйЭлемент = Элементы.Добавить(Колонка.Имя, Тип("ПолеФормы"), Родитель);
		...
		Если Лев(Колонка.Имя, 3) = "ТЧ_" Тогда
			...
			Если КолонкаСвязьПараметровВыбора <> Неопределено Тогда 
				...
				Для каждого СвязьПараметраВыбора Из КолонкаСвязьПараметровВыбора Цикл
					Позиция = СтроковыеФункцииКлиентСервер.НайтиСимволСКонца(СвязьПараметраВыбора.ПутьКДанным, ".");
			...
			КолонкиАссоциации = ИнформацияПоКолонкам.НайтиСтроки(Отбор);
			
			Если КолонкиАссоциации.Количество() = 1 Тогда
				
				КолонкаУровень2 = ВременнаяТЗ.Колонки.Найти("ФЛ_" + КолонкиАссоциации[0].ИмяКолонки);
			...
			ИначеЕсли КолонкиАссоциации.Количество() > 1 Тогда
				...
				Для каждого КолонкаАссоциация Из КолонкиАссоциации Цикл
